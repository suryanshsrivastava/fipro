    ...
    for crawled in files:
        try:
            ...
            results.append(ProcessingResult(
                source_file=crawled.filepath, bank=parser.bank_name,
                total_transactions=len(txns), successful=len(txns),
                failed=0, duplicates_skipped=0, transactions=txns,
                errors=[], warnings=[],
            ))
            ...
            results.append(ProcessingResult(
                source_file=crawled.filepath, bank=crawled.metadata.get('bank', 'UNKNOWN'),
                total_transactions=0, successful=0, failed=0, duplicates_skipped=0,
                transactions=[], errors=[str(e)], warnings=[],
            ))
...
def extract_raw_dataframe(filepaths: List[str]) -> pd.DataFrame:
    ...
    records = [{
        ...
        'amount': t.amount, 'transaction_type': t.transaction_type.value,
        'source_bank': t.source_bank, 'source_file': t.source_file, 'balance': t.balance,
...
def route_file_to_parser(filename: str, df: pd.DataFrame, parsers: List) -> object:
