...
def test_mixed_basic_pipeline_tags_external_account_payments(tmp_path):
    ...
    assert len(run.results) == 3
    all_transactions = [transaction for result in run.results for transaction in result.transactions]
    assert any(
        transaction.description == "CREDIT CARD PAYMENT HDFC CARD"
        and transaction.external_account_name == "CREDIT_CARD"
        for transaction in all_transactions
    )

    csv_text = (output_dir / "goodbudget_export.csv").read_text(encoding="utf-8")
    expected_csv = (case_dir / "expected_goodbudget.csv").read_text(encoding="utf-8")
    assert _normalize_csv(csv_text) == _normalize_csv(expected_csv)


def _normalize_csv(text: str) -> str:
    lines = [line.strip() for line in text.strip().splitlines() if line.strip()]
    return "\n".join(sorted(lines[1:])) if len(lines) > 1 else ""
